...
/// A structure representing the arguments for running a model in JavaScript.
#[derive(Serialize, Deserialize)]
struct JsRunArgs {
    variables: std::collections::HashMap<String, usize>,
}

/// A structure representing the visibility of variables in JavaScript.
#[derive(Serialize, Deserialize)]
struct JsVisibility {
    input: String,
    output: String,
...
/// A structure representing the output of the prover in WebAssembly.
#[derive(Serialize, Deserialize)]
#[wasm_bindgen]
...
impl WasmProverOutput {
    /// Gets the output of the prover.
    ///
    /// # Returns
    ///
    /// A `Result` containing the output as a `JsValue` on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the output cannot be serialized.
    #[wasm_bindgen(getter)]
    ...
    /// Gets the proof generated by the prover.
    ///
    /// # Returns
    ///
    ...
    /// Gets the verifier index used by the prover.
    ///
    /// # Returns
    ///
    /// A `String` containing the verifier index.
    #[wasm_bindgen(getter)]
...
/// A structure representing a model in WebAssembly.
#[wasm_bindgen]
...
impl WasmModel {
    /// Loads an ONNX model from bytes and prepares it for execution.
    ///
    /// # Arguments
    ///
    /// * `bytes` - A byte slice containing the ONNX model.
    /// * `run_args` - The arguments for running the model.
    ///
    /// # Returns
    ///
    /// A `Result` containing the loaded model and symbol values on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the model cannot be loaded, input facts cannot be set,
    /// or the model cannot be converted to a typed model.
...
#[wasm_bindgen]
impl WasmModel {
    /// Creates a new `WasmModel` instance.
    ///
    /// # Arguments
    ///
    /// * `model_bytes` - A byte slice containing the ONNX model.
    /// * `run_args_js` - The arguments for running the model in JavaScript.
    /// * `visibility_js` - The visibility settings for the model variables in JavaScript.
    ///
    /// # Returns
    ///
    /// A `Result` containing the new `WasmModel` instance on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the run arguments or visibility settings are invalid,
    /// or if the model cannot be loaded or parsed.
    ...
    /// Saves the model to a byte vector.
    ///
    /// # Returns
    ///
    /// A `Result` containing the serialized model as a byte vector on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the model cannot be serialized.
...
/// A structure representing the prover system in WebAssembly.
#[wasm_bindgen]
pub struct WasmProverSystem {
    inner: ProverSystem,
}

/// A structure representing the verifier system in WebAssembly.
#[wasm_bindgen]
...
impl WasmProverSystem {
    /// Creates a new `WasmProverSystem` instance.
    ///
    /// # Arguments
    ///
    /// * `model_bytes` - A byte slice containing the ONNX model.
    ...
    /// # Returns
    ///
    /// A `Result` containing the new `WasmProverSystem` instance on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the model cannot be loaded or the prover system cannot be created.
    #[wasm_bindgen(constructor)]
    pub fn new(
        model_bytes: &[u8],
        run_args_js: JsValue,
    ...
    /// # Arguments
    ///
    /// * `inputs_js` - The inputs for the model in JavaScript.
    ///
    /// # Returns
    ///
    /// A `Result` containing a `Promise` that resolves to the proof on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the inputs are invalid or the proof cannot be generated.
    #[wasm_bindgen]
    ...
    /// Gets the verifier system associated with the prover system.
    ///
    /// # Returns
    ///
    /// A `WasmVerifierSystem` instance.
    #[wasm_bindgen]
...
impl WasmVerifierSystem {
    /// Serializes the verifier index to a base64-encoded string.
    ///
    /// # Returns
    ///
    /// A `Result` containing the serialized verifier index as a base64-encoded string on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the verifier index cannot be serialized.
    #[wasm_bindgen]
    ...
    /// Deserializes a verifier index from a base64-encoded string.
    ///
    /// # Arguments
    ///
    /// * `data` - A base64-encoded string containing the serialized verifier index.
    ///
    /// # Returns
    ///
    /// A `Result` containing the deserialized `WasmVerifierSystem` instance on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the base64 string cannot be decoded or the verifier index cannot be deserialized.
    #[wasm_bindgen]
    ...
    /// Gets the verifier index as a JSON string.
    ///
    /// # Returns
    ///
    /// A `Result` containing the verifier index as a JSON string on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the verifier index cannot be serialized.
    ...
    /// Verifies a proof against the verifier index.
    ///
    /// # Arguments
    ///
    /// * `proof_js` - The proof to be verified in JavaScript.
    /// * `public_inputs_js` - The public inputs for the proof in JavaScript.
    /// * `public_outputs_js` - The public outputs for the proof in JavaScript.
    ///
    /// # Returns
    ///
    /// A `Result` containing a boolean indicating the verification result on success, or an error on failure.
    ///
    /// # Errors
    ///
    /// This function will return an error if the proof, public inputs, or public outputs are invalid,
